...
YT_List = 
{
	...
	--2号 Modbus 遥调控制单元（包含4路遥调控制）
	{"RTU/GGIO2.SetPnt1", ".setMag.f"},--遥调控制，32位浮点数型
	{"RTU/GGIO2.SetPnt2", ".setMag.f"},--遥调控制，32位浮点数型
	{"RTU/GGIO2.SetPnt3", ".setMag.f"},--遥调控制，32位浮点数型
	{"RTU/GGIO2.SetPnt4", ".setMag.f"},--遥调控制，32位浮点数型

	--3号 Modbus 遥调控制单元（包含4路遥调控制）
	{"RTU/GGIO3.SetPnt1", ".setMag.f"},--遥调控制，32位浮点数型
	{"RTU/GGIO3.SetPnt2", ".setMag.f"},--遥调控制，32位浮点数型
	{"RTU/GGIO3.SetPnt3", ".setMag.f"},--遥调控制，32位浮点数型
	{"RTU/GGIO3.SetPnt4", ".setMag.f"} --遥调控制，32位浮点数型
}
...
MB_List = 
{
	...
	--2号 Modbus 遥调控制单元（包含4路遥调控制）
	{
		--9600,无校验,1位停止位,功能码"10",modbus地址0x03,应答最长等待100ms,分包间隔时间1000ms
		com = {"BAUDRATE_9600","NoneParity","StopBit_1","10",0x03,100,1000},  
		data = 
		{
			{"RTU/GGIO2.SetPnt1",0x0000, "F_ABCD"}, --遥调控制，写2个16bit寄存器，构成32位ABCD类型浮点数
			{"RTU/GGIO2.SetPnt2",0x0002, "F_ABCD"}, --遥调控制，写2个16bit寄存器，构成32位ABCD类型浮点数
			{"RTU/GGIO2.SetPnt3",0x0004, "F_ABCD"}, --遥调控制，写2个16bit寄存器，构成32位ABCD类型浮点数
			{"RTU/GGIO2.SetPnt4",0x0006, "F_ABCD"}  --遥调控制，写2个16bit寄存器，构成32位ABCD类型浮点数
		}
	},
	--3号 Modbus 遥调控制单元（包含4路遥调控制）
	{
		--9600,无校验,1位停止位,功能码"10",modbus地址0x04,应答最长等待100ms,分包间隔时间1000ms
		com = {"BAUDRATE_9600","NoneParity","StopBit_1","10",0x04,100,1000},  
		data = 
		{
			{"RTU/GGIO3.SetPnt1",0x0000, "F_ABCD"}, --遥调控制，写2个16bit寄存器，构成32位ABCD类型浮点数
			{"RTU/GGIO3.SetPnt2",0x0002, "F_ABCD"}, --遥调控制，写2个16bit寄存器，构成32位ABCD类型浮点数
			{"RTU/GGIO3.SetPnt3",0x0004, "F_ABCD"}, --遥调控制，写2个16bit寄存器，构成32位ABCD类型浮点数
			{"RTU/GGIO3.SetPnt4",0x0006, "F_ABCD"}  --遥调控制，写2个16bit寄存器，构成32位ABCD类型浮点数
		}
	}
}
