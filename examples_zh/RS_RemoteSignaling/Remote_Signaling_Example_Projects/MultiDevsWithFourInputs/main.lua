...
YX_List = 
{
	...
	--2号 Modbus IO 输入监测单元（包含4路输入监测）
	{"RTU/GGIO2.Ind1", ".stVal"}, --开关量IO输入监测，0或1
	{"RTU/GGIO2.Ind2", ".stVal"}, --开关量IO输入监测，0或1
	{"RTU/GGIO2.Ind3", ".stVal"}, --开关量IO输入监测，0或1
	{"RTU/GGIO2.Ind4", ".stVal"}, --开关量IO输入监测，0或1
	--3号 Modbus IO 输入监测单元（包含4路输入监测）
	{"RTU/GGIO3.Ind1", ".stVal"}, --开关量IO输入监测，0或1
	{"RTU/GGIO3.Ind2", ".stVal"}, --开关量IO输入监测，0或1
	{"RTU/GGIO3.Ind3", ".stVal"}, --开关量IO输入监测，0或1
	{"RTU/GGIO3.Ind4", ".stVal"}  --开关量IO输入监测，0或1
}
...
MB_List = 
{
	...
	--2号 Modbus IO 输入监测单元（包含4路输入监测）
	{
		--9600,无校验,1位停止位,功能码"01",modbus地址0x06,应答最长等待100ms,分包间隔时间1000ms
		com = {"BAUDRATE_9600","NoneParity","StopBit_1","01",0x06,100,1000},  
		data = 
		{
			{"RTU/GGIO2.Ind1",0x0000,"BIT",0}, --开关量IO输入监测，读线圈，返回0或1
			{"RTU/GGIO2.Ind2",0x0001,"BIT",0}, --开关量IO输入监测，读线圈，返回0或1
			{"RTU/GGIO2.Ind3",0x0002,"BIT",0}, --开关量IO输入监测，读线圈，返回0或1
			{"RTU/GGIO2.Ind4",0x0003,"BIT",0}  --开关量IO输入监测，读线圈，返回0或1
		}
	},
	--3号 Modbus IO 输入监测单元（包含4路输入监测）
	{
		--9600,无校验,1位停止位,功能码"01",modbus地址0x07,应答最长等待100ms,分包间隔时间1000ms
		com = {"BAUDRATE_9600","NoneParity","StopBit_1","01",0x07,100,1000},  
		data = 
		{
			{"RTU/GGIO3.Ind1",0x0000,"BIT",0}, --开关量IO输入监测，读线圈，返回0或1
			{"RTU/GGIO3.Ind2",0x0001,"BIT",0}, --开关量IO输入监测，读线圈，返回0或1
			{"RTU/GGIO3.Ind3",0x0002,"BIT",0}, --开关量IO输入监测，读线圈，返回0或1
			{"RTU/GGIO3.Ind4",0x0003,"BIT",0}  --开关量IO输入监测，读线圈，返回0或1
		}
	}
}
